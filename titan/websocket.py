...
def handle_message(table, connection_id, event_body, apig_management_client):
    """
    Handles messages sent by an agent. Looks up all connections
    ...
    try:
        ...
        host = item_response["Item"]["host"]
        logger.info("Got user name %s.", host)
    except ClientError:
        logger.exception("Couldn't find user name. Using %s.", host)
    ...
    message = f"{host}: {event_body['msg']}".encode()  # utf-8
